...
| Codex CLI 0.152.1 + native Windows + local CLI launcher | Implementation and synthetic tests only; no reviewed live verification evidence yet | **Candidate/unverified** | Production `run` refuses to arm; `verify-local-hook` may arm only after explicit interactive confirmation on native Windows with exactly 0.152.1 |
| Windows v1 process/session binding | Named-pipe broker with `PIPE_REJECT_REMOTE_CLIENTS`, current-user DACL, `GetNamedPipeClientProcessId`, token SID match, and Toolhelp ancestry using `(PID, creation time, user SID)` | Implemented, not live-verified | Unrelated, stale, mismatched-SID, PID-reused, malformed, or shutdown clients receive no decision |
...
The internal binding protocol is `permission-binding-v1`; it is distinct from Codex's observed `permission-request-v1` hook protocol. It uses a length-prefixed JSON envelope with a maximum request of 1 MiB plus 4 KiB, a 256-byte response maximum, one request per connection, a two-second connection read/write timeout, and a maximum of 16 active connections. No credential, environment, authentication material, or arbitrary file content is sent.
