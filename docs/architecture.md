...
The current repository is pre-alpha. The verified production tuples are Linux/local CLI/Codex CLI 0.151.0 and native Windows/local CLI/Codex CLI 0.153.2 and 0.154.0, each with `PermissionRequest`, `permission-request-v1`, observed `Bash`, and one-request structured allow. Linux 0.153.0 was inspected but has no independently identifiable reviewed live evidence in this checkout and remains experimental/unverified. Native Windows 0.152.1 remains candidate/unverified; Linux 0.153.4 remains an experimental/unverified requested target. No live hook configuration is installed. The adapter baseline is a policy for attempting newer stable releases; it is not the verified-version registry.
...
8. The broker independently validates the bound child identity, secret, platform/session tuple, supported runtime request schema, and cwd. The hook prints the documented one-request structured allow only after broker allow; otherwise it prints nothing. Codex applies its normal hook composition and permission semantics.
...
Linux remains the first verified target because the evidence is Ubuntu-based and ordinary inherited terminal I/O is straightforward to validate there. Native Windows Codex 0.153.2 and 0.154.0 now have user-supplied live verification for their exact local CLI tuples, while Windows 0.152.1 remains candidate/unverified. The implementation and focused tests cover process resolution, `-c` hook registration, child environment behavior, request parsing, response handling, interruption, exit status, argument forwarding, and temporary test configuration without writing the user's live Codex home; the supplied 0.154.0 excerpt does not independently restate each of those mechanics.
...
macOS remains unsupported. The user-supplied Windows 0.153.2 evidence covers executable resolution, inherited terminal behavior, environment inheritance, process creation/ancestry, quoting, exit codes, configuration paths, and the exact PermissionRequest exchange for the local CLI surface. The supplied 0.154.0 excerpt records its exact tuple, redacted counters, command result, exits, and cleanup, without claiming an additional startup/baseline artifact. Neither Windows entry inherits Linux review claims or transfers to Windows 0.152.1 or other surfaces. VS Code/IDE, desktop, remote, container, WSL, SSH-hosted IDE, and Codex cloud surfaces are also unsupported and unverified.
