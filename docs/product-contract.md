...
This is the intended normative contract for `codex-autoapprover`. **MUST** and **MUST NOT** are mandatory requirements. **SHOULD** and **SHOULD NOT** are strong defaults that require a documented reason to change. **MAY** describes an optional capability. The verified production tuples are Linux + local CLI launcher + Codex CLI 0.151.0 and native Windows + local CLI launcher + Codex CLI 0.153.2 and 0.154.0. Linux 0.153.0 was inspected but has no independently identifiable reviewed live evidence in this checkout and remains experimental/unverified. Native Windows 0.152.1 remains candidate/unverified; Linux 0.153.4 remains an experimental/unverified requested tuple.
...
## Distribution wrapper and startup update contract

The documented distribution entry point is `codexa` or `codex-autoapprover run`. Direct invocation of the user's official `codex` command MUST remain untouched; the installer and launcher MUST NOT shadow, replace, or recursively resolve it.

Startup update checks MUST be enabled by default. This is an automatic check, not automatic installation: installing a newer autoapprover release MUST require explicit interactive consent. A future `--no-startup-update-checks` option or equivalent user-scoped setting MUST provide a way to disable startup checks, while `update --check` MUST perform an explicit bounded check without installing. A changed installed Codex version MUST trigger an applicable-release check before the installed implementation is attempted. While compatibility is unresolved, each wrapper launch MUST recheck when allowed by server-directed rate-limit backoff and cross-process coordination. A check timeout or GitHub unavailability MUST continue with the installed version and current automatic compatibility policy.

When an applicable release is found before Codex starts, an interactive terminal MUST offer exactly these conceptual choices: **Install**, **Continue this time**, and **Skip this release**, showing the current/new versions, compatibility status, and release-notes link. Noninteractive input MUST select the nonblocking continue path without prompting or consuming Codex input. **Skip this release** MUST be scoped to the exact installed Codex version and offered release version; a later Codex version change or different applicable release MUST be reconsidered. Decline, skip, no applicable release, backoff, unavailable service, malformed metadata, incompatible assets, and failed activation MUST continue with the known-good installed payload.

The updater MUST run only in the wrapper before Codex starts. It MUST NOT run in the hook protocol process, prompt through hook stdin, or write hook stdout. Installation activation MUST finish before the single Codex child starts; a failed activation MUST preserve the known-good payload. If activation requires a wrapper restart, it MUST use one internal marker bound to the candidate release/digest, consume and clear that marker before Codex launch, and bypass further restart if the marker is repeated or invalid. Once Codex has started, the wrapper MUST never replay commands or restart the session. It MUST preserve arguments, cwd, terminal I/O, ordinary approval fallback, and child exit status.

The wrapper MUST distinguish capability-check success, a genuine authenticated hook exchange, no hook invocation, hook incompatibility/transport failure, and command execution failure. A successful Codex exit without a hook exchange MUST NOT establish compatibility. Local observations MUST remain local observations and MUST NOT promote project-wide reviewed/verified support. A hook failure MUST preserve ordinary approval fallback and record a fixed-category local compatibility outcome.

The initial distribution design targets native Windows x64 (`x86_64-pc-windows-msvc`, proposed Windows 10 22H2+) and native Linux x64 (`x86_64-unknown-linux-gnu`, proposed glibc 2.31+). These are proposed targets, not validated installable support, until the M4/M6 acceptance work succeeds. The stable launcher uses user-scoped paths and immutable versioned payloads with an active pointer so Windows cannot require replacement of a locked running binary. Existing Cargo/package-manager installations MUST remain usable, MUST never be silently overwritten, and MUST receive instructions or an explicit migration offer.

The release channel MUST authenticate metadata and target hashes before activation, enforce expiration and rollback protection, and treat metadata as release selection only. It MUST NOT override the existing compatibility registry, process/session binding, cwd, schema, tool, exact-command, broker, or fail-closed authorization checks. The complete decision tables and trust/library records are canonical in [distribution-roadmap.json](distribution-roadmap.json).
...
Unarmed or rejected targets MUST run with automation disabled or fail with a clear error. Exactly Linux local CLI Codex 0.151.0 and native Windows local CLI Codex 0.153.2 and 0.154.0 are verified in this checkout. Native Windows 0.152.1 remains candidate/unverified; Linux 0.153.0 and Linux 0.153.4 remain experimental/unverified. Newer eligible stable versions MAY be attempted automatically but remain experimental/unverified until separately reviewed. The broker applies the bound version, platform, surface, protocol, and observed `Bash` checks independently of inherited environment metadata, so a forged, stale, malformed, or runtime-incompatible request receives no decision and falls back to normal approval.

The verified entries record release `0.1.0`, hook event `PermissionRequest`, project protocol/schema marker `permission-request-v1`, response behavior one-request structured allow, and isolated live end-to-end verification. Compatibility with these exact releases MUST NOT be inferred for another release, macOS, other Windows versions, VS Code/IDE, desktop, remote, container, WSL, SSH-hosted IDE, or Codex cloud. The maintained exclusion mechanism MUST be used for explicitly incompatible tuples; it MUST not be replaced by an undocumented numeric exception.
